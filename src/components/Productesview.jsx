...
const Productesview = ({title,suptitle,data}) => {
  ...
  function convertDriveLinkToImageUrl(driveUrl) {
    ...
    return (
        ...
        <div className='container mx-auto w-full my-10'>
            <div >
            <div className='flex  items-center gap-2 mt-10'> <p className='w-[20px] h-[40px] rounded-md bg-Secondary2'></p><p className='font-semibold text-[20px] text-Secondary2'>{suptitle}</p></div>
            <p className='text-[36px] font-semibold my-7'>{title}</p>

            </div>
          
        <div className='flex justify-between w-full items-center flex-wrap gap-y-10 mb-20'>
          {data.map((hamza)=>(
              <div key={hamza.productId} className=' ml-2  flex flex-col  h-[350px] w-[300px]'>
              <div className='  flex flex-col items-start h-[350px] w-fit'>
             <div className='bg-Secondary group cursor-pointer flex justify-center items-center h-[250px] w-[270px] relative overflow-hidden rounded-md'>
                 ...
                 <img src={convertDriveLinkToImageUrl(hamza.CoverImage)} alt="" className='w-[170px] h-[150px] object-contain' />
            ...
            <div className='flex items-center '> <ProductRating rating={hamza.rating ||4 } /> <span className='text-Text1  ml-3'>({hamza.rated || 65})</span></div>
